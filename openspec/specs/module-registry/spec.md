...
(`needsGPS`, `needsCompass`, `needsCamera`, `needsMap`) et son rendu.
...
`GEOFENCE` + auto-validation Quiz + `forceDraw`) et preview Studio (meme
mecanisme de bypass, entree Studio), avec flag triche propage au scoring.
Rejouabilite sans perte de stats, accessibilite voix haute/contraste et
autres modes sont en roadmap, hors socle.
