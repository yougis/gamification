...
- **Désélection au clic sur le fond** : cliquer le pane vide du canvas efface `sel` (+ `selMulti`) via les setters gardés, sans toucher aux positions ni recadrer.
- **Synchronisation graphe ↔ liste** : une seule vérité de sélection ; sélectionner dans la liste surligne le nœud du canvas (et inversement, la liste rend l'élément visible) ; la désélection nettoie les deux côtés.
- **Liste triée et réordonnançable** : ordre naturel du jeu par défaut (rang topologique depuis les dépendances d'activation, ex æquo au même rang triés en ordre naturel alphabétique/numérique) ; drag-and-drop dans la liste pour réordonner manuellement (persisté) + action « Tri naturel » pour restaurer l'ordre calculé.
...
- `studio-authoring` : précision de l'exigence « Canvas graphe détaillé » — sélection (simple et multiple) stable sans boucle de re-rendu, zoom/pan standard non contrarié par un recadrage automatique, canvas monté avec taille mesurable ; ET retour au rendu par défaut : boîtes uniformes, liens bas→haut, libellés texte simples, plus aucune distinction visuelle `draft` ni style d'arête custom sur le canvas (delta spec à suivre) ; ET désélection au clic sur le fond, synchronisation graphe ↔ liste, tri naturel et drag-and-drop de la liste des étapes.

## Impact

- **Code** : `studio/src/App.tsx` (`zoneGraphe`, `noeuds`/`aretes`, `onNodesChange`, `onSelectionChange`, `fitView`, libellés, `onPaneClick`), `studio/src/components/NodeList.tsx` (témoin indépendant + drag-and-drop et tri naturel de la liste), `studio/src/styles/theme.css` (règles `.studio-flow` sur nœuds et textes d'arêtes à retirer ; chrome contrôles/minimap/fond conservé).
